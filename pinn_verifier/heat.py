...
class CROWNHeatVerifier():
    ...
    def compute_sin_initial_conditions_bounds(
        ...
        # bounds on linear term and constant from crown for the model
        u_theta_coeffs_ubs, u_theta_consts_ubs, u_theta_coeffs_lbs, u_theta_consts_lbs = u_theta.layer_CROWN_coefficients[-1]
        
        # Encode interval by 0,1,2 (hard coded values, may change as needed)
        encoded_interval = domain_bounds.detatch().clone().cpu()
        def interval_encode(x):
            if x >= -1 and x < -0.5:
                return 0
            if x >= -0.5 and x < 0.5:
                return 1
            if x >= 0.5 and x <= 1:
                return 2
        encoded_interval = encoded_interval.apply_(interval_encode).to(int)


        # This for loop assumes all values in encoded_interval is 0,1,2, and the lower bounds and upper bounds are sorted correctly.
        for i, (idx_l, idx_u) in enumerate(encoded_interval):
            if idx_l == idx_u:
                if idx_l == 1:
                    domain_bounds[i] = torch.sin(domain_bounds[i])
                else:
                    domain_bounds[i] = torch.sin(torch.flip(domain_bounds[i,1],dims=0))
            else:
                if idx_l == 0:
                    if idx_u == 1:
                        domain_bounds[i] = -1,torch.sin(domain_bounds[i]).max()
                    else:
                        domain_bounds[i] = -1,1
                else:
                    domain_bounds[i] = torch.sin(domain_bounds[i]).min(), 1

        dim = domain_bounds.shape[0]
        N = 2 ** dim

        # Generating N by dim index list to extract values from domain_bounds
        index_list = [[int(c) for c in format(i, f"0{dim}b")] for i in range(N)]
        result = torch.prod(domain_bounds[[list(range(dim))]*N, index_list], dim=1)
        sin_pi_x_coeffs_ubs, sin_pi_x_coeffs_lbs = result.max().item(), result.min().item()
